...
extendSetting (char **setting, const char *value, int prepend) {
  if (value && *value) {
    if (!*setting) {
      ...
      size_t len = strlen(*setting) + 1 + strlen(value) + 1;
      char *newSetting = malloc(len);
      ...
      if (prepend) {
        snprintf(newSetting, len, "%s,%s", value, *setting);
      } else {
        snprintf(newSetting, len, "%s,%s", *setting, value);
      }
...

  {
    ...
    for (index=0; index<info->optionCount; ++index) {
      ...
      if (entry->word) {
	    ...
	    size_t len = length + strlen(entry->word) + 1;
	    name = malloc(len);
	    if (name)
	      snprintf(name, len, "%s%s", prefix, entry->word);
          }
...
processEnvironmentVariables (
  ...
  for (optionIndex=0; optionIndex<info->optionCount; ++optionIndex) {
    ...
    if ((option->flags & OPT_Environ) && option->word) {
      size_t len = prefixLength + 1 + strlen(option->word) + 1;
      char name[len];
      snprintf(name, len, "%s_%s", prefix, option->word);
