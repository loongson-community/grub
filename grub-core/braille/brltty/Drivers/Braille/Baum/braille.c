...
logBaumPowerdownReason (BaumPowerdownReason reason) {
  ...
  snprintf(&buffer[length], sizeof (buffer) - length, "Baum Powerdown%n", &length);
  for (entry=reasonTable; entry->bit; ++entry) {
    if (reason & entry->bit) {
      int count;
      snprintf(&buffer[length], sizeof (buffer) - length, "%c %s%n", delimiter, entry->explanation, &count);
      length += count;
      delimiter = ',';
    }
  }
